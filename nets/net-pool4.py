...
def fcn(split, num_classes=None):
    ...
    # score
    n.score_fr = L.Convolution(n.pool4, num_output=num_classes, kernel_size=1, pad=0,
                               param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])

    # deconv : 16x
    n.upscore = L.Deconvolution(n.score_fr,
                                convolution_param=dict(num_output=num_classes, kernel_size=32,
...
def make_net():
    with open('../model/voc-fcn-pool4.prototxt', 'w') as f:
        f.write(str(fcn('deploy', 21)))
    with open('../model/nyud-fcn-pool4.prototxt', 'w') as f:
        f.write(str(fcn('deploy', 40)))
    with open('../model/cityscapes-fcn-pool4.prototxt', 'w') as f:
