...
class cityscapes:
    ...
    def get_dset(self, split):
        ...
        '''
        if split == 'train':
            dataset = open('{}/leftImg8bit_trainvaltest/leftImg8bit/train.txt'.format(self.dir)).read().splitlines()
        else:
            dataset = open('{}/leftImg8bit_trainvaltest/leftImg8bit/val.txt'.format(self.dir)).read().splitlines()
        return [(item.split('/')[0], item.split('/')[1]) for item in dataset]

    def load_image(self, split, city, idx):
        im = Image.open('{}/leftImg8bit_trainvaltest/leftImg8bit/{}/{}/{}_leftImg8bit.png'.format(self.dir, split, city, idx))
        return im
    ...
    def load_label(self, split, city, idx):
        ...
        """
        label = Image.open('{}/gtFine_trainvaltest/gtFine/{}/{}/{}_gtFine_labelIds.png'.format(self.dir, split, city, idx))
